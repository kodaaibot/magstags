...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Theme switch — the header toggle is hidden on phones (base.css), so the
  // overlay carries one. Icons/labels are shown per theme via CSS.
  var themeWrap = document.createElement('div');
  themeWrap.className = 'nav-overlay-theme';
  ...
  themeToggle.innerHTML =
    ...
    '<span class="lbl-dark">Dark mode</span><span class="lbl-light">Light mode</span>';
  themeToggle.addEventListener('click', function() {
    if (typeof window.toggleTheme === 'function') window.toggleTheme();
  });
  themeWrap.appendChild(themeToggle);
  overlay.appendChild(themeWrap);
